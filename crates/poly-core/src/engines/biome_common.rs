...
/// Map one [`AnalyzerDiagnostic`] emitted by biome to a poly [`Diagnostic`].
///
/// * **code**: `category().name()` — e.g. `"lint/correctness/noUnknownProperty"`.
///   This is the stable string users target in `[rules.<code>]` in `poly.toml`.
/// * **severity**: biome `Severity` → poly `Severity`.  Per-rule severity
