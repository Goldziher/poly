...
Skips have three sources: a language nothing in the run lints, no matching engine, or a file poly
deliberately withholds a write from. All of them appear in the summary, in the `--format json`
payload, and in the `--deny-skips` / `--max-skips` budget. A run that verified less than it claims
exits **2**, distinct from the **1** that means real findings.
